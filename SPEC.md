...
Client-side chat toolkits (components/GuidedToolkit.tsx over the shared CoachChat) following the conversational-arc content model (Parent Coach pattern) — no stage ever renders as a monolithic outcome screen:
1. Branch question(s) with emoji chips (one clarifying question per branch).
2. "What's really going on": 2–3 short validating coach bubbles delivered sequentially with typing pauses (plain language, never clinical); a spot illustration accompanies the first.
3. "A strategy to try": ONE named strategy per path as a styled in-chat card — name, what-it-is in 2 sentences, then 2–3 label:benefit "What makes it work" bullets. A tappable "How do I start?" chip advances; remaining content is never auto-dumped.
4. Personalization: a per-toolkit tailoring question (3–4 emoji-chip options with coach ack lines), then a pick-list bubble (4–5 emoji options) whose selection is echoed into the close.
5. Close: a highlighted "Say this"/"Try tonight" card incorporating the pick, a one-line encouragement bubble, then related link, consult CTA, and disclaimer as separate small bubbles, plus restart.
Pacing rule (enforced in components/CoachChat.tsx): no single coach turn exceeds ~60 words or contains more than one card — longer text auto-splits into sequential bubbles at sentence boundaries. Spot illustrations are original inline SVGs in the site palette (timer, book nook, backpack, calm corner), decorative and aria-hidden. Content lives in app/resources/toolkits/toolkits.ts. No backend, no AI calls, no form submission — pure static/client state. Linked from Resources and the sitemap.
...
### Classroom Strategy Finder (/resources/classroom, /resources/classroom/[slug])
Symptom-first navigation for teachers, limited to academic-engagement situations. Index headlined "Click what you're seeing" with a tappable grid of 12 situations: not-turning-in-work, off-task-non-disruptive, off-task-disruptive, unmotivated, disorganized, rushing-through-work, cant-work-independently, blurting-and-interrupting, unprepared-for-class, avoids-writing-tasks, gives-up-quickly, chronically-late-to-class. Each page (data in app/resources/classroom/classroom.ts, rendered by components/SituationPage.tsx): "What it looks like" (3–4 observable, neutral bullets) → "What's often underneath" (2–3 questions to consider, never conclusions) → "Try first this week" (4–5 low-lift strategies with short how-tos) → "Building the habit" (3–4 longer-arc strategies) → "When to loop in others" (family / grade team / counselor or support staff, especially with distress, big changes, or cross-setting patterns) → printable-template + related links → teacher-framed CTA line → standard disclaimer. Per-page metadata is written for search intent ("Strategies When a Student Isn't Turning In Work"). Content rules (hard): classroom/instructional practices in plain teacher language; no clinical, diagnostic, therapy, or behavior-analysis terms; no tier/PBIS branding; no reward systems framed as treatment; situations describe behavior, never character. Index + all 12 pages on /tools (Teachers · Strategy guide) and in the sitemap; the Resources Teachers section links the Finder.
